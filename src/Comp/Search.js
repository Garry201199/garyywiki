...
const Search = () => {
  ...
  return (
    <div>
  <div className="bar">
    <input value={info || ''} onChange={(e) => setInfo(e.target.value)} className="searchbar" type="text" placeholder="Search" />
  ...
  {updatedData && 
    ...
    <ul style={{listStyleType: 'none'}} key={k.pageID}>
        <li > <a href={k.pageLink}> <h3>{k.title}</h3></a> </li>
        <li > <span ><a href={k.pageLink} style={{ wordWrap: 'break-word'}}> <h3>{k.pageLink}</h3></a></span></li>
        <li >
        <p  style={{ wordWrap: 'break-word'}} dangerouslySetInnerHTML={{__html: k.snippet}}></p>
